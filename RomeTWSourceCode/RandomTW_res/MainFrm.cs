...
namespace RandomTW
{
    public partial class MainFrm : Form
    {
        ...
        void WriteNewTGA(int W, int H, Color[,] clrData, string ss){
            ...
            FS.WriteByte(0);
            FS.WriteByte(0);
            FS.WriteByte((byte)Paloma.ImageType.UNCOMPRESSED_TRUE_COLOR);
        ...
        void GenUnits()
        {
            ...
            for (i = 0; i < 19; i++)
            {
                ...
                SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                    UnitTypes[utypes_g[i]].model1 + ", 20");
                SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                    UnitTypes[utypes_g[i]].model2 + ", 30");
                SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                    UnitTypes[utypes_g[i]].model3 + ", 40");
                SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                    UnitTypes[utypes_g[i]].model4 + ", max");
                ...
                if (SMap.FIs[i])
                {
                    for (k = 0; k < unit4faction; k++)
                    {
                        ...
                        SW.WriteLine("texture\t\t\t\t" + "merc" + ", " + //UnitDir + @"/textures/" + 
                            sss);
                        SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                            UnitTypes[utypes_f[i * unit4faction + k]].model1 + ", 20");
                        SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                            UnitTypes[utypes_f[i * unit4faction + k]].model2 + ", 30");
                        SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                            UnitTypes[utypes_f[i * unit4faction + k]].model3 + ", 40");
                        SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                            UnitTypes[utypes_f[i * unit4faction + k]].model4 + ", max");
                        SW.WriteLine("model_tri\t\t\t" + "400, 0.5f, 0.5f, 0.5");
                        SW.WriteLine();
                        SW.WriteLine();
                    }
            ...
            for (i = 0; i < AORs; i++)
            {
                for (k = 0; k < unit4AOR; k++)
                {
                    ...
                    SW.WriteLine("texture\t\t\t\t" + "merc" + ", " + //UnitDir + @"/textures/" + 
                        sss);
                    SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                        UnitTypes[utypes_l[i * unit4AOR + k]].model1 + ", 15");
                    SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                        UnitTypes[utypes_l[i * unit4AOR + k]].model2 + ", 30");
                    SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                        UnitTypes[utypes_l[i * unit4AOR + k]].model3 + ", 40");
                    SW.WriteLine("model_flexi\t\t\t" + //UnitDir + @"/models/" + 
                        UnitTypes[utypes_l[i * unit4AOR + k]].model4 + ", max");
                    SW.WriteLine("model_tri\t\t\t" + "400, 0.5f, 0.5f, 0.5");
                    SW.WriteLine();
                    SW.WriteLine();
                }
            ...
            for (i = 0; i < AORs; i++)
            {
                for (k = 0; k < unit4AOR; k++)
                {
                    ...
                    SW.WriteLine("attributes       " + UnitTypes[utypes_l[i * unit4AOR + k]].attributes);
            ...
            for (i = 0; i < SMap.AORs; i++)
            {
                ...
                for (j = 0; j < unit4AOR; j++)
                {
                    SW.WriteLine("\tunit " + unames_l[i * unit4AOR + j] + ",\t\t\texp 0 cost 1500 replenish 0.11 - 0.35 max 3 initial 1");
                }
            }
            SW.WriteLine();
            SW.Close();
        ...
        bool LoadLetters()
        {
            ...
            if (File.Exists(UnitDir + "//letters.txt"))
            {
                ...
                if (!SR.EndOfStream)
                {
                    ...
                    SMap.fb = new string[flab.Count];
                    for (i = 0; i < flab.Count; i++)
                    {
                        SMap.fb[i] = (string)(flab[i]);
                    }
                }
                else return false;
                if (!SR.EndOfStream)
                {
                    ...
                    SMap.lb = new string[flab.Count];
                    for (i = 0; i < flab.Count; i++)
                    {
                        SMap.lb[i] = (string)(flab[i]);
                    }
                }
                else return false;
                if (!SR.EndOfStream)
                {
                    ...
                    SMap.bb = new string[flab.Count];
                    for (i = 0; i < flab.Count; i++)
                    {
                        SMap.bb[i] = (string)(flab[i]);
                    }
                }
                else return false;
                if (!SR.EndOfStream)
                {
                    ...
                    SMap.aa = new string[flab.Count];
                    for (i = 0; i < flab.Count; i++)
                    {
                        SMap.aa[i] = (string)(flab[i]);
                    }
                }
                else return false;
                SR.Close();
