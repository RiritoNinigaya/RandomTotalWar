...
public class StratMap
{
    ...
    public int[] clm_hmd_Dw = { 255, 255, 255, 255, 150, 10, 10 };
    ...
    //public string[] Facs = { "huns", "goths", "sarmatians", "vandals", "franks",
    //                        "saxons", "celts", "sassanids", "roxolani", "ostrogoths", 
    //                        "romano_british", "slavs", "burgundii", "berbers", "alemanni", 
    //                        "empire_east", "empire_west", "empire_east_rebels", "empire_west_rebels"};
    public string[] FacsNames = { "romans_julii", "romans_brutii", "romans_scipii", "egypt", "seleucid",
                            "carthage", "parthia", "gauls", "germans", "britons", 
                            "greek_cities", "macedon", "pontus", "armenia", "dacia", 
                            "numidia", "scythia", "spain", "thrace"};
    //public string[] FacsNames = { "huns", "goths", "sarmatians", "vandals", "franks",
    //                        "saxons", "celts", "sassanids", "roxolani", "ostrogoths", 
    //                        "romano_british", "slavs", "burgundii", "berbers", "alemanni", 
    //                        "empire_east", "empire_west", "empire_east_rebels", "empire_west_rebels"};
    ...
    public void GenRegions(int W, int H, int RN, ref TWReg[] A, ref Color[,] C)
    {
                ...
                if ((myFtrData[rec.x, rec.y] != 1) && (rec.k < 150))
                {
                        ...
                        || ((rec.val + 5 < RegVal[rec.x, rec.y]) && (AvMap[rec.x, rec.y] != 1)))
                    {
                        ...
                        if (rec.x - 1 >= 0)
                        {
                            ...
                            rec2.val = m + 2.0 * RndAr[rec2.x, rec2.y] * rec.k / 100; if (AvMap[rec2.x, rec2.y] == 1) rec2.val += 50;
        ...
        for (i = 0; i < RN; i++)
        {
            for (j = i + 1; j < RN; j++)
            {
                if (MainClmOfRegs[i] == MainClmOfRegs[j])
                {
                    if (Math.Pow((A[i].X - A[j].X), 2) + Math.Pow((A[i].Y - A[j].Y), 2) < 1000 * (1.0 * W / 200) * (1.0 * H / 150))
                    {
                        RegsToRegs[i, j] = true;
                        RegsToRegs[j, i] = true;
                    }
    ...
    public void recAORReg(int ri, int ai)
    {
        AORsOfRegs[ri] = ai;
        int i;
        for (i = ri + 1; i < RN; i++)
    ...
    public void CreateDscReg(int RN, TWReg[] Regs, string ss)
    {
        ...
        for (i = 0; i < RN; i++)
        {
            ...
            if (New)
            {
                ...
                + "\t5");
                SW.WriteLine("\t" + (1 + (int)((PopOfRegs[i] - 2000) / 10000 * 15)));
                SW.WriteLine();
            }
            else
            {
